...
def _tensor_to_canny(tensor, th_low, th_high):
    """
    ...
    for i in range(tensor.shape[0]):
        ...
        canny_image = _process_single_image(image_tensor, th_low, th_high)
...
class CannyDiffimeInterp(DiffimeInterp):
    """The quadratic model"""
    def __init__(self, path='models/raft_model/models/rfr_sintel_latest.pth-no-zip', config=None, canny_th_low=50, canny_th_high=200):
        super(CannyDiffimeInterp, self).__init__(path, config, init_diff=False)
    ...
    def forward(self, I1, I2, F12i, F21i, t, folder=None):
        store_latents_path = os.path.join(self.config.store_path, folder, "latents")
        ...
        caption1 = generate_caption(im1, max_words=3, style=extract_style_name(folder))
        caption2 = generate_caption(im2, max_words=3, style=extract_style_name(folder))
        d_I1c = self.pipeline(caption1, image=w_I1c_img).images[0].resize(self.config.test_size)
        d_I2c = self.pipeline(caption2, image=w_I2c_img).images[0].resize(self.config.test_size)
